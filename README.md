...
- **The post-action protocol** — after every change: note → index → affected docs pages → area log → worksite `state.md`. Success criterion: *an AI with zero context, given the files, knows exactly where things stand and what to do next.* A Stop hook enforces the habit.
