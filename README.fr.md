...
- **Le protocole post-action** — après chaque modif : note → index → pages docs concernées → log d'aire → `state.md` du chantier. Critère de réussite : *une IA sans aucun contexte, avec les seuls fichiers, sait où on en est et quoi faire ensuite.* Un hook Stop ancre l'habitude.
